var FourBoardView = (function (_super) {
    _super.uiView = Pai.filterImgAddVersion(_super.uiView)
    function FourBoard() {
        FourBoard.super(this);
        this.ballArr = []
        this.zhuangID = -1
    ...
    Laya.class(FourBoard, 'FourBoard', _super);
    var _prototype = FourBoard.prototype
    // 初始化
    _prototype.init = function () {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            var n = Pai.round.ChipLevels[i]
            c.tag = n
            c.skin = 'comp/4renchang/' +n+ '.png?v='+Pai.version
    ...
    // 开始抢庄动画
    _prototype.startRobAnimate = function () {
        this.controlRobAnimate = 1
        for (var i = 0; i < this.playerArr.length; i++) {
            var flag = this.playerArr[i].getChildByName('ringFlag')
    ...
    _prototype.sendPaiAnimate = function (random) {
        ...
        var posArr = [{ x: 0, y: 116 }, { x: 224, y: 0 }, { x: 440, y: 116 }, { x: 224, y: 259 }]
            ...
            for (var i = 0; i < 4; i++) {
                (function (num) {
                    ...
                    Laya.Tween.to(self.paiBox._childs[arr[num]], { x: posArr[arr[num]].x, y: posArr[arr[num]].y }, (num + 1) * 100, Laya.Ease.linearIn, Laya.Handler.create
                        (self, self.haha), num * 100);
                })(i)
            }
        }, 3000);
    }
    _prototype.haha = function () {
        console.log(1)
    }
    // 更新玩家状态
    ...
    _prototype.showAllTableMoney = function (allPlayerCoinMap) {
        ...
        for (var key in allPlayerCoinMap) {
            ...
            // } else if (allPlayerCoinMap[key] >= 0) {
            //     str = '万'
            // } else {
            //     allPlayerCoinMap[key] = Math.abs(allPlayerCoinMap[key]) * 1000
            //     str = ''
            // }
    ...
    _prototype.showUserTableMoney = function (currentPlayerCoinMap) {
        ...
        for (var key in currentPlayerCoinMap) {
            ...
            // } else if(currentPlayerCoinMap[key] >= 0){
            //     str = '万'
            // }else{
            //     str = ''
            // }
