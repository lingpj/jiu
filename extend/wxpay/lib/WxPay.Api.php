...
//		curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,2);//严格校验
		...
		if($useCert == true){
			...
			//add by zjh 原来的证书地址因为不是绝对地址，会出现curl 52错误，不要尝试使用dirname等函数，只能用绝对路径
			$cert_path = EXTEND_PATH.'wxpay'.DS.'cert'.DS;
			$sslcert = $cert_path . 'apiclient_cert.pem';
			$sslkey = $cert_path . 'apiclient_key.pem';
